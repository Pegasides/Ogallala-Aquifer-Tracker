...
const BASE = 'https://pegasides.github.io/Ogallala-Aquifer-Tracker/';
let failures = 0;
const results = [];
...
async function waitForPages() {
  ...
  throw new Error('GitHub Pages did not become reachable in the allotted time.');
}
...
try {
  ...
  check('V3 home loads', (await page.title()).includes('Version 3'), await page.title());
  ...
  check('V3 home navigation order', JSON.stringify(homeNav.slice(0,4)) === JSON.stringify(['V3 Home','Interactive Timeline','Groundwater Window','My Aquifer']), homeNav.slice(0,4).join(' → '));
  ...
  check('Mobile V3 home avoids page-level horizontal overflow', await mpage.evaluate(() => document.documentElement.scrollWidth <= document.documentElement.clientWidth + 1), `scroll=${await mpage.evaluate(() => document.documentElement.scrollWidth)}, client=${await mpage.evaluate(() => document.documentElement.clientWidth)}`);
